...
```{r,eval=F,echo=T}
sample_aggregated_segvals_output_full<-formSampleMatrixFromRawGDCData(tcga_files = target_files_aml,format = "TARGET")
saveRDS(sample_aggregated_segvals_output_full,"aml_sample_matched_input_matrix.rds")
```
...
cor(t(sample_aggregated_segvals_output_full))  %>% 
...
```
```{r eval=F}
#ggplot(data = ggplotmatrix_minimap ) + #geom_tile() + theme_void()
