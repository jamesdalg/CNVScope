...
#' @param partitions the partitions to which the jobs are to be scheduled, in
#' order of priority.
...
#'
#' @export
calcVecLMs<-function(bin_data,use_slurm=F,job_finished=F,slurmjob=NULL,n_nodes=NULL,cpus_on_each_node=2,memory_per_node="2g",walltime="4:00:00",partitions="ccr,quick")
{
  ...
  if(use_slurm){
    if(job_finished)
    {
      ...
      lm_test_sjob <- rslurm::slurm_apply(function(x,y) -log(summary(lm(unlist(bin_data_df[,y])~unlist(bin_data_df[,x])))$coefficients[2,4]), bin.pairs, jobname = 'CNVScope_LM_apply',
                                          nodes = n_nodes, 
                                          cpus_per_node = cpus_on_each_node, 
                                          submit = T,slurm_options = list(partition=partitions,mem=memory_per_node,time=walltime))
      return(lm_test_sjob)
    }
  }
}