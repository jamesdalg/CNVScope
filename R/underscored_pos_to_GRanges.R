...
#' @export
underscored_pos_to_GRanges<-function(underscored_positions=NULL,extended_data=NULL)
{  GRanges(seqnames = as.character(plyr::ldply(sapply(underscored_positions,
                             ...
                             as.numeric(as.character(plyr::ldply(sapply(underscored_positions,
                                                                        function(x) strsplit(as.character(x),"_",fixed=T)
                             ),rbind)["2"][,1])),end = as.numeric(as.character(plyr::ldply(sapply(underscored_positions,
                                                                                                  function(x) strsplit(as.character(x),"_",fixed=T)
                             ),rbind)["3"][,1]))),... = extended_data)
}
